...
- `.github/workflows/` — `deploy.yml` (Pages) + `fetch-telegram.yml` (cron lun/jeu 10h UTC)
...
3. **Ne jamais éditer `public/Assets/messages.json` à la main** : ce fichier est régénéré par `fetch-telegram.yml` (lundis/jeudis 10h UTC). Toute édition manuelle sera écrasée au prochain run.
